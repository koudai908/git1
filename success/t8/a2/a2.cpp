...
int main(int argc, char *argv[])
{
    ...
    qDebug() << "pnode->nVersion: " << pnode->nVersion << endl;

    //pnode->PushMessage("ping");

    char sendBuf[0x10000] = "hello, server";
    int nBytes = send(pnode->hSocket, sendBuf, sizeof(sendBuf), 0);
    if (nBytes > 0)
    {
        qDebug() << "send success! " << endl;
    }
...
CNode* ConnectNode1(CAddress addrConnect, const char *pszDest, int64 nTimeout)
{
    ...
    if (ConnectSocket(addrConnect, hSocket))
    {
        ...
        // debug print
        qDebug() << "connected to: " << addrConnect.ToString().c_str() << endl;
