...
{
    qDebug() << "CWallet::AvailableCoins start! " << endl;
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->IsFinal())
            {
                qDebug() << "pcoin->IsFinal() true! pass! " << endl;
            }
            else
            {
                qDebug() << "pcoin->IsFinal() false! reject!" << endl;
                continue;
            }

            if (fOnlyConfirmed && pcoin->IsConfirmed())
            {
                qDebug() << "fOnlyConfirmed true, and  pcoin->IsConfirmed() true! pass!" << endl;
            }
            else
            {
                qDebug() << "fOnlyConfirmed true, and  pcoin->IsConfirmed() false! reject!" << endl;
                continue;
            }
...
/*
            ...
            {
                qDebug() << "pcoin->IsCoinBase() && pcoin->GetBlocksToMaturity() > 0 true! reject!" << endl;
                continue;
            }
            else
            {
                qDebug() << "pcoin->IsCoinBase() && pcoin->GetBlocksToMaturity() > 0 false! pass!" << endl;
            }
*/
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                {
                    qDebug() << "!(pcoin->IsSpent(i)) && IsMine(pcoin->vout[i]) && pcoin->vout[i].nValue > 0 true! pass!" << endl;
                    vCoins.push_back(COutput(pcoin, i, pcoin->GetDepthInMainChain()));
                }
                else
                {
                    qDebug() << "!(pcoin->IsSpent(i)) && IsMine(pcoin->vout[i]) && pcoin->vout[i].nValue > 0 true! reject!" << endl;
                }
            }
...
{
    qDebug() << "CWallet::CreateTransaction start! " << endl;
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                {
                    qDebug() << "SelectCoins success! " << endl;
                }
                else
                {
                    qDebug() << "SelectCoins fail! " << endl;
                    return false;
                }
